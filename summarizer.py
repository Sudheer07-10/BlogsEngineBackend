...
def _fallback_summarize(text: str, max_sentences: int = 3, persona: dict = None) -> str:
    ...
    # Basic Persona styling for fallback
    if persona:
        vibes = ", ".join(persona.get("vibes", [])).lower()
        if "genz" in vibes or "energetic" in vibes:
            # Just a tiny bit of flavor so it doesn't look totally robotic
            summary = f"No cap: {summary} (Insight from {persona.get('personaName', 'Alex')})"
...
def summarize(text: str, max_sentences: int = 3, persona: dict = None, fallback_title: str = "New Insight", model_name: str = "gemini-2.0-flash") -> str:
    """
    ...
    p_role = persona.get("personaRole", "Content Expert") if persona else "Content Expert"
    p_vibes = ", ".join(persona.get("vibes", ["energetic"])) if persona else "energetic"
    ...
    for attempt in range(2):
        try:
            ...
            # If everything else is missing, we'll handle gracefully.
            return result
            ...
            if "429" in str(e) and attempt == 0:
                ...
                print(f"⚠️  [Summarizer] Quota hit for {p_name}. Needs {wait_hint} cooling off. (Retrying in 3s...)")
                time.sleep(3)
                continue
